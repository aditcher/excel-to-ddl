...
def format_sql_value(v, col_type):
    """Return a properly SQL-quoted value string based on inferred column type."""
    if v is None or str(v).strip() == "":
        return "NULL"
    s  = str(v).strip()
    t  = col_type.upper()
    numeric_hints = ["INT","FLOAT","DECIMAL","NUMERIC","NUMBER","BIT","TINYINT","BOOLEAN"]
    if any(x in t for x in numeric_hints):
...
def build_dml(cols, table_name, dialect, rows, row_limit):
    """
    Generate all DML artefacts for the given dialect:
      1. Batch INSERT statements
      2. Snowflake COPY INTO template  (Snowflake only)
      3. CSV staging export note
      4. UPDATE statement template
      5. MERGE / UPSERT statement template
    Returns a single SQL string with clear section headers.
    """
    q   = lambda n: quote_name(n, dialect)
    tn  = table_name.upper() if dialect == "Oracle" else table_name
    schema = "PUBLIC." if dialect == "Snowflake" else ""
    ...
    # Honour row limit (0 = all)
    sample_rows = rows if row_limit == 0 else rows[:row_limit]
    col_names   = ", ".join(q(c["name"]) for c in cols)

    pk = next((c for c in cols if c["is_pk"]), cols[0] if cols else None)

    out = []
    out.append(
        f"-- ============================================================\n"
        f"-- Excel → DDL Automator  —  DML Output\n"
        f"-- Generated : {ts}\n"
        f"-- Dialect   : {dialect}\n"
        f"-- Table     : {tn}   Rows: {len(sample_rows):,}\n"
        f"-- ============================================================\n"
    )
    ...
    out.append(
        f"\n-- ────────────────────────────────────────────────────────────\n"
        f"-- SECTION 1: Batch INSERT statements  ({len(sample_rows):,} rows)\n"
        f"-- ────────────────────────────────────────────────────────────\n"
    )

    if dialect == "SQL Server (T-SQL)":
        # T-SQL supports multi-row VALUES
        CHUNK = 1000
        for start in range(0, len(sample_rows), CHUNK):
            chunk = sample_rows[start:start+CHUNK]
            val_lines = []
            for row in chunk:
                vals = [format_sql_value(
                    row[i] if i < len(row) else None, c["type"])
                    for i, c in enumerate(cols)]
                val_lines.append(f"    ({', '.join(vals)})")
            out.append(
                f"INSERT INTO {q(tn)} ({col_names})\nVALUES\n"
                + ",\n".join(val_lines) + f"{semi}\n"
            )
    else:
        # One INSERT per row — portable across all dialects
        for row in sample_rows:
            vals = [format_sql_value(
                row[i] if i < len(row) else None, c["type"])
                for i, c in enumerate(cols)]
            out.append(
                f"INSERT INTO {schema}{q(tn)} ({col_names})\n"
                f"VALUES ({', '.join(vals)}){semi}\n"
            )

    # ── 2. Snowflake COPY INTO template ───────────────────────────────────
    if dialect == "Snowflake":
        stage_name = f"@%{tn}"
        out.append(
            f"\n-- ────────────────────────────────────────────────────────────\n"
            f"-- SECTION 2: Snowflake COPY INTO (bulk-load template)\n"
            f"-- ────────────────────────────────────────────────────────────\n"
            f"-- Step 1: Create or use a named stage\n"
            f"CREATE OR REPLACE STAGE {tn}_STAGE\n"
            f"  FILE_FORMAT = (TYPE = 'CSV' FIELD_OPTIONALLY_ENCLOSED_BY='\"'\n"
            f"                 SKIP_HEADER = 1 NULL_IF = ('NULL','N/A',''))\n"
            f"  COMMENT = 'Auto-generated by Excel → DDL Automator';\n\n"
            f"-- Step 2: Upload your CSV to the stage (run in SnowSQL or Snowflake CLI)\n"
            f"-- PUT file://<path-to-csv> @{tn}_STAGE;\n\n"
            f"-- Step 3: Load into table\n"
            f"COPY INTO PUBLIC.{q(tn)}\n"
            f"FROM @{tn}_STAGE\n"
            f"FILE_FORMAT = (FORMAT_NAME = '{tn}_STAGE')\n"
            f"ON_ERROR = 'CONTINUE'\n"
            f"PURGE = TRUE;\n\n"
            f"-- Step 4: Validate load results\n"
    ...
    out.append(
        f"\n-- ────────────────────────────────────────────────────────────\n"
        f"-- SECTION 3: CSV Staging Export\n"
        f"-- ────────────────────────────────────────────────────────────\n"
        f"-- Use the 'Save Staged CSV' button in the toolbar to export\n"
        f"-- the cleaned dataset as a UTF-8 CSV ready for bulk loading.\n"
        f"-- Column order matches the DDL schema exactly.\n"
        f"-- Header row: {', '.join(c['name'] for c in cols)}\n"
    )

    # ── 4. UPDATE template ────────────────────────────────────────────────
    pk_col  = q(pk["name"]) if pk else q(cols[0]["name"])
    set_clause = ",\n    ".join(
        f"{q(c['name'])} = <new_{c['name']}>"
        for c in cols if not c["is_pk"]
    )
    out.append(
        f"\n-- ────────────────────────────────────────────────────────────\n"
        f"-- SECTION 4: UPDATE statement template\n"
        f"-- ────────────────────────────────────────────────────────────\n"
        f"-- Replace <...> placeholders with real values or bind parameters.\n"
        f"UPDATE {schema}{q(tn)}\n"
        f"SET\n    {set_clause}\n"
        f"WHERE {pk_col} = <pk_value>{semi}\n"
    )

    # ── 5. MERGE / UPSERT ─────────────────────────────────────────────────
    non_pk_cols   = [c for c in cols if not c["is_pk"]]
    update_clause = ",\n        ".join(
        f"T.{q(c['name'])} = S.{q(c['name'])}" for c in non_pk_cols
    )
    insert_src    = ", ".join(f"S.{q(c['name'])}" for c in cols)

    if dialect == "Snowflake":
        merge_sql = (
            f"MERGE INTO {schema}{q(tn)} AS T\n"
            f"USING {schema}{q(tn)}_STAGE_VIEW AS S\n"
            f"    ON T.{pk_col} = S.{pk_col}\n"
            f"WHEN MATCHED THEN UPDATE SET\n"
            f"        {update_clause}\n"
            f"WHEN NOT MATCHED THEN INSERT ({col_names})\n"
            f"    VALUES ({insert_src}){semi}\n"
        )
    elif dialect == "MySQL":
        # MySQL uses INSERT … ON DUPLICATE KEY UPDATE
        dup_clause = ",\n    ".join(
            f"{q(c['name'])} = VALUES({q(c['name'])})" for c in non_pk_cols
        )
        merge_sql = (
            f"-- MySQL UPSERT via INSERT … ON DUPLICATE KEY UPDATE\n"
            f"INSERT INTO {q(tn)} ({col_names})\n"
            f"VALUES (<row_values>)\n"
        ...
        dup_clause = ",\n        ".join(
            f"{q(c['name'])} = EXCLUDED.{q(c['name'])}" for c in non_pk_cols
        )
        merge_sql = (
            f"-- PostgreSQL UPSERT via INSERT … ON CONFLICT\n"
            f"INSERT INTO {q(tn)} ({col_names})\n"
            f"VALUES (<row_values>)\n"
        ...
        merge_sql = (
            f"MERGE INTO {q(tn.upper())} T\n"
            f"USING (SELECT <values> FROM DUAL) S\n"
            f"    ON (T.{pk_col} = S.{pk_col})\n"
            f"WHEN MATCHED THEN UPDATE SET\n"
            f"        {update_clause}\n"
            f"WHEN NOT MATCHED THEN INSERT ({col_names})\n"
    ...
    else:  # SQL Server (T-SQL)
        merge_sql = (
            f"MERGE INTO {q(tn)} AS T\n"
            f"USING (VALUES (<row_values>)) AS S ({col_names})\n"
            f"    ON T.{pk_col} = S.{pk_col}\n"
            f"WHEN MATCHED THEN UPDATE SET\n"
            f"        {update_clause}\n"
            f"WHEN NOT MATCHED THEN INSERT ({col_names})\n"
    ...
    out.append(
        f"\n-- ────────────────────────────────────────────────────────────\n"
        f"-- SECTION 5: MERGE / UPSERT template\n"
        f"-- ────────────────────────────────────────────────────────────\n"
        + merge_sql
    )
...
class App(tk.Tk):
    ...
    def _build_config(self, parent):
        ...
        # DML row limit row
        r2b = tk.Frame(card, bg=PANEL)
        r2b.pack(fill="x", padx=10, pady=(0,4))
        tk.Label(r2b, text="DML row limit:", bg=PANEL, fg=TEXT_MUTED,
                 font=SANS).pack(side="left", padx=(0,6))
        self._dml_limit_var = tk.StringVar(value="500")
        ttk.Combobox(r2b, textvariable=self._dml_limit_var,
                     values=["100","500","1000","All rows"],
                     state="readonly", width=12).pack(side="left", padx=(0,20))
        tk.Label(r2b, text="(applies to DML tab INSERTs only)",
                 bg=PANEL, fg=TEXT_DIM, font=SANS_S).pack(side="left")
    ...
    def _build_output(self, parent):
        ...
        for label, cmd in [("  Copy DDL  ", self._copy_ddl),
                            ...
                            ("  Save DML .sql  ", self._save_dml_sql),
                            ("  Save Staged CSV  ", self._save_staged_csv),
        ...
        # DML Output tab
        dml_f = tk.Frame(nb, bg="#0d1117")
        nb.add(dml_f, text="  DML Output  ")
        self._dml_text = tk.Text(
            dml_f, font=MONO, bg="#0d1117", fg="#e6edf3",
            insertbackground="#0d1117", relief="flat", wrap="word",
            padx=14, pady=12,
            spacing1=0, spacing2=0, spacing3=0,
            highlightthickness=0, borderwidth=0,
            ...
            takefocus=0,
            selectbackground=ACCENT, selectforeground="#ffffff")
        self._dml_text.bind("<Button-1>", lambda e: self.after(1, lambda: self.focus_set()))
        self._dml_text.bind("<FocusIn>",  lambda e: self.after(1, lambda: self.focus_set()))
        dml_sy = ttk.Scrollbar(dml_f, orient="vertical", command=self._dml_text.yview)
        self._dml_text.configure(yscrollcommand=dml_sy.set)
        dml_sy.pack(side="right", fill="y")
        self._dml_text.pack(fill="both", expand=True)
        self._dml_text.tag_configure("kw",  foreground=SQL_KW)
        self._dml_text.tag_configure("cmt", foreground=SQL_CMT)
        self._set_dml(
            "-- DML Output will appear here after clicking Generate.\n"
            "-- Includes: Batch INSERTs, Snowflake COPY INTO, UPDATE template, MERGE/UPSERT.\n"
        )
    ...
    def _generate(self):
        ...
        # resolve DML row limit
        lim_str = self._dml_limit_var.get()
        row_limit = 0 if lim_str == "All rows" else int(lim_str)
        ...
        def _do():
            ...
            dml       = build_dml(cols, table_name, dialect, rows, row_limit)
            ...
            def _done():
                ...
                self._gen_btn.enable(bg=SUCCESS, fg="#ffffff")
                self._gen_btn.set_text("  ⚡ Generate DDL + DML  ")
            self.after(0, _done)
        threading.Thread(target=_do, daemon=True).start()

    def _set_dml(self, text):
        self._dml_text.config(state="normal")
        self._dml_text.delete("1.0","end")
        self._dml_text.insert("1.0", text)
        self._dml_text.tag_remove("kw",  "1.0","end")
        self._dml_text.tag_remove("cmt", "1.0","end")
        KWS = ["INSERT","INTO","VALUES","UPDATE","SET","WHERE","MERGE",
               "USING","WHEN","MATCHED","NOT","THEN","COPY","FROM","ON",
               "CONFLICT","DUPLICATE","KEY","SELECT","CREATE","STAGE","PUT",
               "PURGE","REPLACE"]
        for i, line in enumerate(text.split("\n"), 1):
            if line.strip().startswith("--"):
                self._dml_text.tag_add("cmt", f"{i}.0", f"{i}.end")
            else:
                for kw in KWS:
                    start = f"{i}.0"
                    while True:
                        pos = self._dml_text.search(
                            r'\b'+kw+r'\b', start,
                            stopindex=f"{i+1}.0", regexp=True)
                        if not pos: break
                        self._dml_text.tag_add("kw", pos, f"{pos}+{len(kw)}c")
                        start = f"{pos}+{len(kw)}c"
        self._dml_text.config(state="disabled")

    def _copy_dml(self):
        if not self._dml:
            messagebox.showinfo("Nothing to copy","Generate DDL + DML first."); return
        self.clipboard_clear(); self.clipboard_append(self._dml)
        messagebox.showinfo("Copied","DML copied to clipboard.")

    def _save_dml_sql(self):
        if not self._dml:
            messagebox.showinfo("Nothing to save","Generate DDL + DML first."); return
        path = filedialog.asksaveasfilename(
            defaultextension=".sql",
            initialfile=f"{self._table_var.get()}_dml.sql",
            filetypes=[("SQL","*.sql"),("All","*.*")])
        if path:
            with open(path,"w",encoding="utf-8") as f: f.write(self._dml)
            messagebox.showinfo("Saved", f"DML saved to:\n{path}")

    def _save_staged_csv(self):
        if not self._cleaned_rows:
            messagebox.showinfo("Nothing to save","Generate DDL + DML first."); return
        path = filedialog.asksaveasfilename(
            defaultextension=".csv",
            initialfile=f"{self._table_var.get()}_staged.csv",
            filetypes=[("CSV","*.csv"),("All","*.*")])
        if not path: return
        try:
            col_names = [c["name"] for c in self._cols]
            with open(path,"w",newline="",encoding="utf-8") as f:
                w = csv.writer(f)
                w.writerow(col_names)
                for row in self._cleaned_rows:
                    w.writerow([
                        "" if (i >= len(row) or row[i] is None) else row[i]
                        for i in range(len(col_names))
                    ])
            messagebox.showinfo("Saved",
                f"Staged CSV saved to:\n{path}\n\n"
                f"{len(self._cleaned_rows):,} rows  |  {len(col_names)} columns")
        except Exception as e:
